...
def k_data_init2base_CSMAR(folder_name: str | None = None):
    """
    将从国泰安（CSMAR）下载的历史日线数据初始化到数据库中，
    后续日频更新使用akshare
    日期范围1990-12-19至2025-09-12
    文件格式为csv
    源文件列名：
    Stkcd [证券代码] - symbol
    Trddt [交易日期] - trade_date
    Opnprc [日开盘价] - open
    Hiprc [日最高价] - high
    Loprc [日最低价] - low
    Clsprc [日收盘价] - close
    Dnshrtrd [日个股交易股数] - volume
    Dnvaltrd [日个股交易金额] - turnover
    Dsmvosd [日个股流通市值] - 先不用该字段
    Dsmvtll [日个股总市值] - 先不用该字段
    Markettype [市场类型] - 先不用该字段
    Trdsta [交易状态] - 先不用该字段
    PreClosePrice [昨收盘(交易所)] - pre_close
    ChangeRatio [涨跌幅] - chg_pct
